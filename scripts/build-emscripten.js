...
async function main() {
  ...
  // copy the typings of the embind api generated by emscripten and format them
  const typings = path.join(
    __dirname,
    "../packages/cxx-frontend/src/cxx-js.d.ts",
  );

  await $`cp build.em/src/js/cxx-js.d.ts ${typings}`;
  await $`npx prettier --write ${typings}`;
  await $`cp ${typings} ${path.join(__dirname, "../packages/cxx-frontend/dist/")}`;
}
